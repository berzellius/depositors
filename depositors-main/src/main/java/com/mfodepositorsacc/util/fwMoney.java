...
public class fwMoney {
    ...
    public String num2str() {
        return num2str(false, true);
    }

    public String num2str(boolean stripkop, boolean showUnits){
        return num2str(stripkop, showUnits, Type.MONEY);
    }
     ...
     */
    public String num2str(boolean stripkop, boolean showUnits, Type type) {

        if(type.equals(Type.NUMBER_AND_MONEY)){
            showUnits = false;
        }
        ...
        String[][] forms = formsByType(type);
        // получаем отдельно рубли и копейки
        long rub = amount.longValue();
        String[] moi = amount.toString().split("\\.");
        long kop = Long.valueOf(moi[1]);

        if(type.equals(Type.NUMBER) && kop == 0){ // Выводим число. Если дробная часть 0, то нет смысла писать "13 целых 00 сотых"
            showUnits = false;
            stripkop = true;
        }
        ...
        for (int i= 0; i<segments.size(); i++ ) {// перебираем сегменты
            ...
            String add = morph(ri, forms[lev][ 0],forms[lev][1],forms[lev][2])+" ";
            if(lev != 1 || showUnits){
                o += add;
            }
        ...
        if (stripkop) {
            ...
            if(type.equals(Type.NUMBER_AND_MONEY)){
                o = this.amount.setScale(0, RoundingMode.FLOOR).intValue() + " (" + o.trim() + ") " + units + " " + kops + " " + morph(kop, forms[0][0], forms[0][1], forms[0][2]);
            }

            if(type.equals(Type.NUMBER)){
                fwMoney fwKops = new fwMoney(kops);
                o = o + "" + fwKops.num2str(true, false, Type.NUMBER) + " " + morph(kop, forms[0][0], forms[0][1], forms[0][2]);
            }
    ...
    private String[][] formsByType(Type type) {
        switch (type){
            case MONEY:
            case NUMBER_AND_MONEY:
                String[][] formsM = {
        ...
        throw new UnexpectedInputException("There is no forms for this type!");
    }
